...
export class ReviewsService {
  ...
  async createReview(data: CreateReviewDTO) {
    ...
    if (await this.reviewRepository.checkUserReviewExistsForLecture(data.userId, data.lectureId)) throw new ConflictException("User's review already exists for this lecture");
    const lecture = await this.lecturesService.getLectureWithClasstimesById(data.lectureId);
    const new_review = await this.reviewRepository.createReview(data);
  ...
  async updateReviewByUser(data: UpdateReviewDTO) {
    ...
    if (changes.gradeChange !== 0 || changes.loadChange !== 0 || changes.speechChange !== 0) {
      try {
        ...
        console.error(error);
        throw new InternalServerErrorException('Failed to update stats after updating review');
      }
    }
  ...
  async unlikeReview(reviewId: number, userId: number) {
    ...
    await this.getReviewWithId(reviewId); // 존재하지 않으면 불러오는 과정에서 에러가 발생.
    return await this.reviewRepository.unlikeReview(reviewId, userId); // 좋아요를 누르지 않았더라면 변화없으므로 그냥 호출해도 OK
  }
  ...
  async deleteReviewByAdmin(id: number) {
    ...
    const isReported = await this.reportService.checkReportExistsForReview(id);
    if (!isReported) {
    ...
    try {
      ...
      console.error(error);
      throw new InternalServerErrorException('Failed to update stats after deleting review');
    }
