...
export function toCourseWithUnseenReviewDTO(
  ...
  const userLastSeenId = course.userLastSeenReviewOnCourse?.[0]?.lastSeenReviewId ?? 0;

  const courseLastReviewId = course.lastReviewId ?? 0;
  return {
    ...courseWithDeptToCourseDTO(course),
    unseenReview: courseLastReviewId > userLastSeenId   // TODO
  };
}
